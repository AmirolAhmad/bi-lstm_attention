...
def make_sequences(frame: pd.DataFrame, length: int, stride: int = 10) -> tuple[np.ndarray, np.ndarray, dict]:
    ...
    for _, group in frame.sort_values(["cc_num", "unix_time"], kind="mergesort").groupby("cc_num", sort=False):
        ...
        windows.append(np.transpose(view[::stride], (0, 2, 1)).copy())
        labels.append(target[length - 1 :: stride].copy())
    if not windows:
...
def main() -> None:
    ...
    x_train, y_train, train_meta = make_sequences(train, args.sequence_length, args.sequence_stride)
    x_val, y_val, val_meta = make_sequences(val, args.sequence_length, args.sequence_stride)
    x_test, y_test, test_meta = make_sequences(test, args.sequence_length, args.sequence_stride)
